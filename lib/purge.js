...
module.exports = (options) => {
  return getDependencies(options)
    ...
    .then((result) => {
      console.log('Node modules size\t', getSizeInMb(result.allModules) + ' MB');
      console.log('Purged size\t\t', getSizeInMb(result.purged) + ' MB');
      console.log('Difference\t\t', getSizeInMb(result.purged - result.allModules) + ' MB');
    })
    .catch(console.error);
};
