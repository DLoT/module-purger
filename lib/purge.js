...
module.exports = (options) => {
  return getDependencies(options)
    ...
    .then((result) => {
      console.log('Size was \t', getSizeInMb(result.allModules) + ' MB');
      console.log('Purged\t\t', getSizeInMb(result.allModules - result.purged) + ' MB');
      console.log('New size is\t', getSizeInMb(result.purged) + ' MB\n');
    })
    .catch(console.error);
};
